...
export function SecaoBloco4Financeiro({ dados, onChange }: { dados: Record<string, any>; onChange: (d: Record<string, any>) => void }) {
  const set = (patch: Record<string, any>) => onChange({ ...dados, ...patch })
  return (
    <div className="space-y-4">
      <CampoSelect label="4.1 Possui conta bancária em nome do empreendimento?" value={dados.conta_bancaria ?? ''} onChange={v => set({ conta_bancaria: v })}
        opcoes={[{ value: 'sim', label: 'Sim' }, { value: 'nao', label: 'Não' }, { value: 'em_abertura', label: 'Em abertura' }]} />
      <CampoSelect label="4.2 Possui contabilidade regular?" value={dados.contabilidade ?? ''} onChange={v => set({ contabilidade: v })}
      ...
      <CampoSelect label="4.3 Registra receitas, despesas e retiradas?" value={dados.registra_receitas_despesas ?? ''} onChange={v => set({ registra_receitas_despesas: v })}
      ...
      <CampoSelect label="4.4 Elabora fluxo de caixa?" value={dados.fluxo_caixa ?? ''} onChange={v => set({ fluxo_caixa: v })}
        opcoes={[{ value: 'sim_atualiza', label: 'Sim e atualiza' }, { value: 'sim_desatualizado', label: 'Sim, desatualizado' }, { value: 'nao', label: 'Não' }]} />
      <CampoSelect label="4.5 Calcula custos e forma preços?" value={dados.calcula_custos_precos ?? ''} onChange={v => set({ calcula_custos_precos: v })}
      ...
      <CampoTexto label="4.6 Receita média mensal e meses de maior/menor receita" multiline value={dados.receita_media_mensal ?? ''} onChange={v => set({ receita_media_mensal: v })} />
      <CampoSelect label="4.7 Acessou crédito nos últimos 3 anos?" value={dados.acessou_credito ?? ''} onChange={v => set({ acessou_credito: v })}
      ...
      <CampoTexto label="4.8 Fonte, valor, finalidade, prazo e situação do crédito" multiline value={dados.credito_detalhe ?? ''} onChange={v => set({ credito_detalhe: v })} />
      <CampoTexto label="4.9 Necessidade atual de financiamento e capacidade estimada de pagamento" multiline value={dados.necessidade_financiamento ?? ''} onChange={v => set({ necessidade_financiamento: v })} />
      <EscalaMaturidade0a4
        label="Régua de Maturidade do Bloco — Gestão Financeira"
